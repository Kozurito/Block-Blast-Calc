...
def draw_grid(surface, grid, grid_width, grid_height, offset_x=0, offset_y=0, highlight_rows=None, highlight_cols=None):
    ...
    for x in range(grid_width):
        for y in range(grid_height):
            ...
            color = GRID_COLOR
            if y in highlight_rows or x in highlight_cols:
...
def can_place_block(grid, block, x, y):
    ...
    grid_width = len(grid[0]) if grid else 0 #handle empty grid case
        ...
        for col_index, cell in enumerate(row):
            if cell == 1:
                ...
                if (grid_x < 0 or grid_x >= grid_width or
                    ...
                    (grid and grid_y < len(grid) and grid_x < len(grid[grid_y]) and grid[grid_y][grid_x] == 1)): #handle empty grid case and out of bounds after clearing
                    return False
    return True
...
def remove_blank_lines(grid):
    ...
    for y in range(num_rows):
        if all(grid[y][x] == 0 for x in range(num_cols)):
            rows_to_remove.append(y)

    # Identify columns to remove
    if num_rows > 0:
        for x in range(num_cols):
            if all(grid[y][x] == 0 for y in range(num_rows)):
    ...
    if grid and cols_to_remove: # check if the list is not empty
        for row in grid:
...
def find_best_placement(grid, blocks):
    ...
    def solve(current_grid, remaining_blocks, current_moves, current_score):
        ...
        if not remaining_blocks:  # Base case: all blocks placed
            ...
            elif current_score == best_score and remaining_units < min_remaining_units:
                best_move_sequence = current_moves
                min_remaining_units = remaining_units
            return
...
while running:
    ...
    # Draw the three grids for the three possible blocks
